...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        last_hidden, last_cell = _
        enc_hiddens, enc_lenghts = pad_packed_sequence(enc_hiddens)
        enc_hiddens = enc_hiddens.permute(1,0,2)
        
        bidirectional_cell = torch.cat([last_cell[1], last_cell[0]], dim = 1)
        init_decoder_cell = self.c_projection(bidirectional_cell)
        
        bidirectional_hidden = torch.cat([last_hidden[1], last_hidden[0]], dim = 1)
        init_decoder_hidden = self.h_projection(bidirectional_hidden)
