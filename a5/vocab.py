...
class VocabEntry(object):
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        padded_sents = torch.tensor(pad_sents_char(indices, self.char_pad), dtype=torch.long, device=device) #B*max_len_sent*m_word
        B, max_len_sent, m_word = padded_sents.shape
        #return padded_sents.contiguous().view(max_len_sent, B, m_word)
        return padded_sents.contiguous().permute(1,0,2)
